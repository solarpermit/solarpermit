...
def submit_suggestion(request):
    ...
    # temporary
    valid_users = {}
    valid_users['renic'] = [3, 'd3e26a00ababb87994671950e1b65689442b8b4590988ac98c68c37df576e83d']
    valid_users['sundev'] = [667, 'de9d1333c6f378fb']
    ...
    if len(errors) > 0:
        ...
        if len(errors) > 0:
            ...
            # validate user
            if directives['api_username'] not in valid_users or valid_users[directives['api_username']][1] != directives['api_key']:
                output = '<errors>\n\t<error>User validation failure - check api_username and api_key for accuracy</error>\n</errors>'
            else:
                    ...
                    if len(errors) > 0:
                        ...
                        if len(errors) > 0:
                            ...
                            if len(errors) > 0:
                                ...
                                user = User.objects.get(id=valid_users[directives['api_username']][0])
                                is_callout = 0
                                try:
