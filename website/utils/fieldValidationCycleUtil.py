...
class FieldValidationCycleUtil():
    ...
    def pre_validation_check(self, action_category, vote, entity_id):
        ...
        try:
            if answer.approval_status == 'P':
                ...
                if vote == 'down' and len(total_down_votes_while_pending) >= 2:     # 2 down votes to reject
                    status = 'will_reject'
    ...
    def cron_validate_answers(self):
        ...
        today_date = timezone.now()#date.today()#replaced date.today() with the proper timezone.now
