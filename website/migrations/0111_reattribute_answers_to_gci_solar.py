...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        jurisdiction_ids = [193, 133819, 133833, 133839, 133893,
                            133915, 133927, 133929, 133938, 133961,
                            133967, 133971, 133977, 134007, 134008,
                            134011, 134044, 134067, 134074, 134083,
                            134084, 134125, 134159, 134190, 134206,
                            134213, 134231, 134256, 134288, 134307,
                            134309, 134323, 134364, 134376, 134443,
                            134591, 134611, 134633, 134644, 134645,
                            134655, 134696, 134714, 134734, 134743,
                            134752, 134785, 134801, 134846, 134852,
                            134874, 134902, 134930, 134963, 134973,
                            145396, 145827, 146206, 157002, 158009,
                            158028, 158301, 158976, 159101, 159275]
        alphanu11 = orm['auth.User'].objects.get(username = "alphanu11")
        cpf = orm['website.Organization'].objects.get(name = "Clean Power Finance")
        gci_solar = orm['website.Organization'].objects.get(name = "GCI Solar")
        answers = orm['website.AnswerReference'].objects.filter(creator = alphanu11,
                                                                organization = cpf,
                                                                jurisdiction_id__in = jurisdiction_ids)
        for answer in answers:
            answer.organization = gci_solar
            answer.save()
        # technically this should find the newest answer for each
        # jurisdiction and use the org_id from it, but I'm going to
        # assume that in this case it will always be GCI Solar
        for jurisdiction_id in jurisdiction_ids:
            try:
                jurisdiction = orm['website.Jurisdiction'].objects.get(pk = jurisdiction_id)
                jurisdiction.organization = GCI
                jurisdiction.save()
            except:
                pass
    ...
    models = {
        ...
        'website.api_keys': {
            ...
            'revoke_datetime': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
