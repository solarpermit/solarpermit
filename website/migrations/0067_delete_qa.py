...
from website.models import Action, AnswerReference, TemplateQuestion, Question

class Migration(SchemaMigration):

    def forwards(self, orm):
        cursor = connection.cursor()
        cursor.execute("DELETE FROM website_action WHERE 1=1")
        cursor.execute("DELETE FROM website_answerreference WHERE 1=1")
        cursor.execute("DELETE FROM website_templatequestion WHERE 1=1")
        cursor.execute("DELETE FROM website_question WHERE 1=1")
        #also clear the top contributors
        cursor.execute("UPDATE website_jurisdiction SET last_contributed = NULL, last_contributed_by_id = NULL, last_contributed_by_org_id = NULL WHERE 1=1")
        
        '''Action.objects.all().delete()
        AnswerReference.objects.all().delete()
