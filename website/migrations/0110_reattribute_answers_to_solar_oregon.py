...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        jurisdiction_ids = [2958, 2960, 2963, 2965, 2968, 2970, 2971,
                            2972, 2974, 2975, 2976, 2978, 2987, 2988,
                            2989, 2990, 2993, 9564, 9654, 9780, 9809,
                            9941, 10041, 10057, 10196, 10448, 10468,
                            10503, 10538, 10597, 10648, 10811, 10884,
                            11270, 11271, 11291, 11500, 11537, 11762,
                            11792, 146147, 146150, 146151, 161453]
        alphanu11 = orm['auth.User'].objects.get(username = "alphanu11")
        solar_oregon = orm['website.Organization'].objects.get(name = "Solar Oregon")
        answers = orm['website.AnswerReference'].objects.filter(creator = alphanu11,
                                                                jurisdiction_id__in = jurisdiction_ids)
        for answer in answers:
            answer.organization = solar_oregon
            answer.save()
        # technically this should find the newest answer for each
        # jurisdiction and use the org_id from it, but I'm going to
        # assume that in this case it will always be Solar Oregon
        for jurisdiction_id in jurisdiction_ids:
            try:
                jurisdiction = orm['website.Jurisdiction'].objects.get(pk = jurisdiction_id)
                jurisdiction.organization = solar_oregon
                jurisdiction.save()
            except:
                pass
