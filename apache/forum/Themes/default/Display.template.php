...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		if (!$message['member']['is_guest'])
		{
			...
			{
				$lss_val = 'Yes';
				//LSS: Special check for SolarPermit organization
				foreach($message['member']['custom_fields'] as $c)
				{
					if ($c['colname'] == 'cust_displa')
					{
						$lss_val = $c['value'];
						break;
					}
				...
				foreach ($message['member']['custom_fields'] as $custom)
				{
					if ($custom['colname'] == 'cust_displa')
						continue;
					if (empty($custom['placement']) || empty($custom['value']))
					{
						if ($custom['colname'] == 'cust_solarp')
						{
